...
function renderDocCard(doc, fallbackTitle) {
  ...
  card.innerHTML = `
    <h3 class="card-title line-clamp-2">${fallbackTitle}</h3>
    <div class="card-meta">
      <span>${elapsedLabel(doc.updatedAt)} • ${coverageLabelForDoc(doc)} • ${hotspotCountForBrief(doc.id)} hotspots</span>
...
function renderSelectedHotspotDetail(feature) {
  ...
  panel.innerHTML = `
    <div class="hotspot-detail__head">
      ...
      <div class="card-meta">${p.region || "--"} • ${p.category || "--"} ${badge(`risk ${normalizeRisk(p.risk_level)}`, `badge-risk-${normalizeRisk(p.risk_level)}`)}</div>
    </div>
...
async function openDocumentModal(doc) {
  ...
  modalContent.innerHTML = `
    <div class="detail-header">
      ...
      <div class="detail-subheader" id="doc-subheader">${formatTimestamp(doc.updatedAt)} • Coverage: ${coverageLabelForDoc(doc)} • 0 hotspots</div>
  ...
  document.getElementById("doc-subheader").textContent =
    `${formatTimestamp(doc.updatedAt)} • Coverage: ${coverageLabelForDoc(doc)} • ${pins.length} hotspots`;
